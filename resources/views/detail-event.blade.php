        ...
        <div class="card shadow-sm border-0" style="border-radius: 16px;">
            <div class="card-body p-4 p-md-5">
                <div class="mb-4 text-center">
                    <img src="{{ $bannerUrl }}" alt="Banner {{ $event->judul }}"
                        style="max-height: 400px; border-radius: 16px; object-fit: cover; width: 100%; box-shadow: 0 10px 30px rgba(34, 197, 94, 0.15); border: 1px solid rgba(34, 197, 94, 0.1);">
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3 mb-md-0">
                        <p class="mb-2"><i class="bi bi-calendar-event text-primary me-2"></i> <strong>Tanggal:</strong>
                            {{ \Carbon\Carbon::parse($event->tanggal)->format('d F Y') }}</p>
                        @if ($event->jam_mulai)
                            <p class="mb-2"><i class="bi bi-clock text-primary me-2"></i> <strong>Jam:</strong>
                        ...
                        @endif
                        <p class="mb-2"><i class="bi bi-geo-alt-fill text-danger me-2"></i> <strong>Lokasi:</strong>
                            {{ $event->lokasi }}</p>
                    </div>
                    <div class="col-md-6">
                        @if ($event->pembicara)
                            <p class="mb-2"><i class="bi bi-mic-fill text-success me-2"></i> <strong>Pembicara:</strong>
                                {{ $event->pembicara }}</p>
                        @endif
                        <p class="mb-2"><i class="bi bi-people-fill text-warning me-2"></i> <strong>Kuota:</strong>
                            {{ $event->kuota }} (Sisa: <strong
                                class="{{ $remainingQuota <= 0 ? 'text-danger' : 'text-success' }}">{{ $remainingQuota }}</strong>)
                        </p>
                ...
                @if (session('role') === 'admin' || session('role') === 'penyelenggara')
                    <div class="alert text-center py-3"
                        style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 12px; color: #16a34a;">
                        <i class="bi bi-info-circle-fill"></i>
                        Anda login sebagai <strong>{{ session('role') === 'admin' ? 'Admin' : 'Penyelenggara' }}</strong>.
                        Hanya mahasiswa yang dapat mendaftar event.
                    </div>
                @else
                    <div class="card mt-4 shadow-sm"
                        style="border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 12px; background: #f8fafc;">
                        <div class="card-header border-bottom-0 pt-4 pb-0 bg-transparent">
                            <h5 class="mb-0 fw-bold" style="color: #0f172a;"><i
                                    class="bi bi-card-checklist text-primary me-2"></i> Form Pendaftaran Event</h5>
                        </div>
