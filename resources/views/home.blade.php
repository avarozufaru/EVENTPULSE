...
@section('content')
    ...
    @if (count($featuredEvents) > 0)
        <div id="featuredCarousel" class="carousel slide hero-section" data-bs-ride="carousel"
            style="min-height: 420px; overflow: hidden; border-radius: 24px; box-shadow: 0 20px 40px rgba(34, 197, 94, 0.25); border: 1px solid rgba(255, 255, 255, 0.5); margin-bottom: 2rem;">
            <div class="carousel-indicators" style="z-index: 10;">
                @foreach ($featuredEvents as $idx => $fe)
                    <button type="button" data-bs-target="#featuredCarousel" data-bs-slide-to="{{ $idx }}"
                ...
                @foreach ($featuredEvents as $idx => $fe)
                    @php
                        ...
                        if ($fe->banner && $fe->banner !== 'default-banner.jpg') {
                            ...
                            str_contains($feJudulLower, 'ai') ||
                            str_contains($feJudulLower, 'kecerdasan buatan')
                        ) {
                            $feImage = 'images/Seminar AI.png';
                        } elseif (
                            str_contains($feJudulLower, 'ui ux') ||
                            str_contains($feJudulLower, 'ui/ux') ||
                            str_contains($feJudulLower, 'ux') ||
                            str_contains($feJudulLower, 'figma')
                        ) {
                    ...
                    <div class="carousel-item {{ $idx === 0 ? 'active' : '' }}"
                        style="min-height: 420px; background: linear-gradient(rgba(34, 197, 94, 0.6), rgba(21, 128, 61, 0.8)), url('{{ asset($feImage) }}') center/cover no-repeat;">
                        <div class="container text-center text-white py-5 d-flex flex-column justify-content-center align-items-center"
                            style="min-height: 420px; z-index: 5; position: relative;">
                            <span class="badge bg-warning text-dark mb-3 px-3 py-2 rounded-pill fw-bold">🔥 EVENT
                                UNGGULAN</span>
                            <h1 class="fw-bold mb-3 hero-title"
                                style="max-width: 800px; text-shadow: 0 4px 12px rgba(0,0,0,0.3);">{{ $fe->judul }}</h1>
                            <p class="mb-4 fs-5 hero-subtitle"
                                style="max-width: 700px; text-shadow: 0 2px 6px rgba(0,0,0,0.3);">
                                {{ Str::limit($fe->deskripsi, 120) }}</p>
                            <div class="d-flex gap-3 justify-content-center flex-wrap mb-4"
                                style="text-shadow: 0 2px 4px rgba(0,0,0,0.3);">
                                <span class="badge bg-white text-primary bg-opacity-90 border border-light px-3 py-2"><i
                                        class="bi bi-calendar-event"></i>
                                    {{ \Carbon\Carbon::parse($fe->tanggal)->format('d M Y') }}</span>
                                <span class="badge bg-white text-danger bg-opacity-90 border border-light px-3 py-2"><i
                                        class="bi bi-geo-alt-fill text-danger"></i> {{ $fe->lokasi }}</span>
                            </div>
                            <a href="/event/{{ $fe->id }}" class="btn btn-warning btn-lg px-5 fw-bold"
    ...
    <div class="container py-5">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3 mb-4">
            ...
            <div class="search-container">
                ...
                <input type="text" id="eventSearchInput" class="form-control search-input border-primary"
        ...
        @php
            ...
            if (session('role') === 'admin') {
                ...
                $homeBase = '/penyelenggara/home';
            }
        @endphp
        <div class="mb-4">
            <div class="d-flex flex-wrap gap-2 justify-content-start">
                <a href="{{ $homeBase }}"
                    class="btn btn-sm {{ request('category') == '' ? 'btn-primary-gradient text-white' : 'btn-outline-secondary' }} px-4 py-2 rounded-pill shadow-sm">
                    🌐 Semua Event
                </a>
                @foreach ($categories as $cat)
                    <a href="{{ $homeBase }}?category={{ $cat->id }}"
                        class="btn btn-sm {{ request('category') == $cat->id ? 'btn-primary-gradient text-white font-weight-bold' : 'btn-outline-secondary' }} px-4 py-2 rounded-pill shadow-sm">
            ...
            @forelse($events as $event)
                @php
                    ...
                    if ($event->banner && $event->banner !== 'default-banner.jpg') {
                        ...
                        str_contains($judulLower, 'ui ux') ||
                        str_contains($judulLower, 'ui/ux') ||
                        str_contains($judulLower, 'ux') ||
                        str_contains($judulLower, 'figma')
                    ) {
                ...
                <div class="col-lg-4 col-md-6 event-item">
                    ...
                    <div class="card event-card h-100 shadow-sm">
                        ...
                        <div class="event-banner">
                            ...
                            <img src="{{ asset($imagePath) }}" alt="{{ $event->judul }}" class="event-image">

                            <div class="price-badge-container">
                                <span
                                    class="badge price-badge {{ $event->harga == 0 ? 'bg-success' : 'bg-warning text-dark' }}">
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function(e) {
                    ...
                    eventItems.forEach(item => {
                        const title = item.querySelector('.event-title-text').textContent
                            .toLowerCase();
                        const loc = item.querySelector('.event-location-text').textContent
                            .toLowerCase();
                ...
                timers.forEach(timer => {
                    ...
                    if (diff < 0) {
                        ...
                        if (eventDay === today) {
                            ...
                            timer.querySelector('i').className =
