    ...
    <div class="row mb-4">
        <div class="col-md-4 mb-3">
            <div class="glass-box d-flex align-items-center justify-content-between p-4" style="border-left: 4px solid #0ea5e9; height: 100%;">
                ...
                <div style="font-size: 2.5rem; color: rgba(14,165,233,0.4);"><i class="bi bi-calendar-week-fill"></i></div>
            </div>
        </div>
        <div class="col-md-4 mb-3">
    ...
    <div class="row">
        ...
        <div class="col-lg-8 mb-4">
            <div class="glass-box h-100">
                <h5 class="mb-4" style="color: #0f172a;"><i class="bi bi-bar-chart-fill" style="color: #0ea5e9;"></i> Performa Pendaftaran Event</h5>
        ...
        <div class="col-lg-4 mb-4">
            <div class="glass-box h-100 d-flex flex-column justify-content-between p-4">
                <div>
                    <h5 class="mb-4" style="color: #0f172a;"><i class="bi bi-list-task me-2" style="color: #0ea5e9;"></i> Ringkasan Status Event</h5>
                    <div class="d-flex flex-column gap-3">
                        @forelse($eventStats->take(5) as $event)
                        <div class="p-3 rounded" style="background: #f8fafc; border: 1px solid #e2e8f0;">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <h6 class="mb-0 fw-semibold text-truncate" style="max-width: 60%; color: #0f172a;" title="{{ $event->judul }}">{{ $event->judul }}</h6>
                                <div>
                                    @if($event->status === 'published')
                                        <span class="badge" style="background: rgba(16,185,129,0.1); color: #059669; border: 1px solid rgba(16,185,129,0.3); padding: 4px 8px; border-radius: 6px;">Aktif</span>
                                    @elseif($event->status === 'pending')
                                        <span class="badge" style="background: rgba(245,158,11,0.1); color: #d97706; border: 1px solid rgba(245,158,11,0.3); padding: 4px 8px; border-radius: 6px;">Pending</span>
                                    @else
                                        <span class="badge" style="background: rgba(239,68,68,0.1); color: #dc2626; border: 1px solid rgba(239,68,68,0.3); padding: 4px 8px; border-radius: 6px;">Ditolak</span>
                                    @endif
                                </div>
                            </div>
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <span class="text-muted small"><i class="bi bi-people-fill me-1"></i> Pendaftar</span>
                                <span class="fw-medium small" style="color: #0f172a;">{{ $event->total_peserta }} / {{ $event->kuota }}</span>
                            </div>
                            <div class="progress mt-2" style="height: 4px; background: #e2e8f0; border-radius: 2px;">
                                @php
                                    $percent = ($event->kuota > 0) ? min(100, ($event->total_peserta / $event->kuota) * 100) : 0;
                                    $colorStyle = $percent >= 100 ? 'background:#ef4444;' : ($percent >= 80 ? 'background:#f59e0b;' : 'background:#0ea5e9;');
                                @endphp
                                <div class="progress-bar" role="progressbar" style="width: {{ $percent }}%; border-radius: 2px; {{ $colorStyle }}"></div>
                            </div>
                        </div>
                        @empty
                ...
                <div class="text-end pt-4 mt-auto">
                    <a href="/events" class="btn btn-sm w-100" style="border-radius: 8px; border: 1px solid #e2e8f0; padding: 8px; color: #475569;">
